...
```{r include=FALSE}
spainrain = choiceItem(name="Rain",Algeria=list(pts=0,hint="Wrong continent"),
                       Spain=list(pts=2,reward="My Fair Lady!"))
hurricanes= choiceItem(name="Hurricanes", often=list(pts=0),usually=list(pts=1),"hardly ever"=list(pts=2))
```
...
```{r include=FALSE}
f = newMC(pts=5,name="Declaration") # Ordinarily, make this silent
```
...
`r I(numberItem(name="number-choice", ans=pi, tol=1e-3, help="three digits accuracy"))`

* Enter in an integer between 1 and 10 divisible by 7 `r I(numberItem(name="number-choice", ans=7, from=0, to=10, by=1))`

### Free text.

What would you like to write a question about? `r I(textItem(name="question-topic",rows=1))`
